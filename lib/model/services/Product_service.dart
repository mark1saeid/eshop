...
import 'package:eShop/model/data/bannerAds.dart';
import 'package:eShop/model/data/brands.dart';
...
//Adding users' product to cart
addCartToOrders(cartList, orderID, addressList,paymentMethod,totalPrice) async {
  final uEmail = await AuthService().getCurrentEmail();
  final uName = await AuthService().getCurrentName();
  final uPhone = await AuthService().getCurrentPhone();
  ...
  await db
    ...
    {
      'orderID': orderID,
      'orderOwnerName' : uName,
      'orderOwnerEmail' : uEmail,
      'orderOwnerPhone' : uPhone,
